 ...
 * @static
 */
Loan.Utils = Loan.utils = require(__dirname + '/utils');

/**
 * @type Format
 * @static
 */
Loan.Format = Loan.format = require(__dirname + '/format');
...
Loan.prototype.toJSON = function(keepAll) {
  ...
  return _.reduce(fields, function(json, key) {
    if(key === 'data') {
      if(!clone[key]) return json;
      json[key] = _.omit(clone[key], 'payment_plan'); // omit the payment_plan
    } else if(keepAll || clone[key] !== defaults[key]) {
      json[key] = clone[key];
    }
    return json;
  }, {});
 ...
 * @return {Loan}
 */
Loan.prototype.clone = function() {
  var clone = _.cloneDeep(this);
  return new Loan(clone);
};
