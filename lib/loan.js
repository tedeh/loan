...
/**
 * @class Loan
 * @summary Constructor for a Loan
 * @param {Object}  [props]
...
Loan.prototype.canCalculateUnknown = function(prop) {
  var isFiniteNumber = Loan.Utils.isFiniteNumber;

  switch(prop) {

    case 'instalments': return _.every([
      !this.instalments,
      isFiniteNumber(this.principal),
      isFiniteNumber(this.interest_rate),
      isFiniteNumber(this.data.monthly_cost)
    ]);
...
Loan.prototype.calculateUnknown = function(prop) {
  ...
  switch(prop) {
    ...
    case 'instalments':
      ...
      return {instalments: instalments || 0};
...
Loan.prototype.getEffectiveInterestRate = function(method) {
  ...
  function RATE(nper, pmt, pv, fv, type, guess) {
    ...
    while ((Math.abs(y0 - y1) > 0.0000001) && (i < 20 )) {
      rate = Math.abs((y1 * x0 - y0 * x1) / (y1 - y0));
      x0 = x1;
      x1 = rate;
